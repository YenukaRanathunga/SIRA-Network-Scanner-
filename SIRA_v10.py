"""
SIRA v10.0 — Security Intelligence & Recon Agent
Advanced Network, Vulnerability & Threat Intelligence Platform
Built with Python + PyQt6

S.I.R.A — Security Intelligence & Recon Agent

v10.0 NEW FEATURES:
  • Network Topology Map — interactive canvas graph after device discovery
      - Router node at centre, devices as coloured nodes by status
      - Click any node to select/scan that device
      - Zoom & pan with mouse wheel + drag
      - Export topology as PNG image
      - Real-time node status refresh when uptime monitor is running
  • Real-Time Traffic Dashboard — live scrolling charts
      - Packets/sec scrolling line chart (last 60 seconds)
      - Protocol breakdown live bar chart (TCP/UDP/ICMP/Other)
      - Top Talkers table — ranked by packet count with live update
      - Bytes/sec bandwidth graph
      - All powered by matplotlib + captured_packets engine
  ─── INHERITED FROM v9.0 ───────────────────────────────────────
  • Risk Score Engine — 0-100 threat score with colour gauge
  • Scan History & Diff — SQLite history, compare any two scans
  • GeoIP Intelligence — country/city/ISP/ASN lookup
  • PDF Report Export — professional reportlab PDF
  • AI Threat Brief — Anthropic Claude-powered analysis
  ─── INHERITED FROM v8.0 ───────────────────────────────────────
  • Full Wireshark-level packet capture with .pcap export
  • CVE/NVD vulnerability scanning + Exploit-DB cross-reference
  • Network device discovery with uptime monitoring
  • SSL/TLS certificate inspection
  • SYN stealth scan + async TCP/UDP
"""
...
#  FILE LOGGER SETUP
# ─────────────────────────────────────────────
_LOG_DIR = Path.home() / "SIRA_logs"
_LOG_DIR.mkdir(exist_ok=True)
_log_file = _LOG_DIR / f"sira_{datetime.now().strftime('%Y%m%d')}.log"

file_logger = logging.getLogger("SIRA")
...
# ── Matplotlib (for traffic dashboard) ──────────────────────────────────────
...
try:
    ...
    pass  # Dashboard shows a friendly install prompt


# ─────────────────────────────────────────────
#  THEME v10.0 — Professional Security Tool
# ─────────────────────────────────────────────
DARK_BG        = "#0a0e17"
PANEL_BG       = "#111827"
SIDEBAR_BG     = "#0d1321"
...
class ScanWorker(QThread):
    ...
    def query_nvd_enhanced(self, keyword, port):
        ...
        try:
            ...
            headers = {'User-Agent': 'SIRA/8.0 Security Scanner'}
...
# ═══════════════════════════════════════════════════════════════════════════════
#  NETWORK TOPOLOGY MAP CANVAS  (v10.0)
#  Pure PyQt6 painter — no extra dependencies
# ═══════════════════════════════════════════════════════════════════════════════
import math as _math

class TopologyNode:
    """Represents one device node on the topology canvas."""
    RADIUS = 28

    STATUS_COLORS = {
        "ONLINE":   ("#22c55e", "#052e16"),
        "OFFLINE":  ("#ef4444", "#2a0a0a"),
        "UNSTABLE": ("#f59e0b", "#1a0f00"),
        "ROUTER":   ("#00e5ff", "#003344"),
        "GATEWAY":  ("#7c3aed", "#1a0a2e"),
    }

    def __init__(self, ip, label, status="ONLINE", vendor="", node_type="host"):
        self.ip        = ip
        self.label     = label
        self.status    = status.upper()
        self.vendor    = vendor
        self.node_type = node_type   # "router", "host", "gateway"
        self.x         = 0.0
        self.y         = 0.0
        self.selected  = False
        self.hovered   = False
    ...
    def contains(self, px, py) -> bool:
        return _math.hypot(px - self.x, py - self.y) <= self.RADIUS + 4

    def color_pair(self):
        if self.node_type == "router":
            return self.STATUS_COLORS["ROUTER"]
        if self.node_type == "gateway":
            return self.STATUS_COLORS["GATEWAY"]
        return self.STATUS_COLORS.get(self.status, ("#64748b", "#111827"))


class NetworkTopologyCanvas(QWidget):
    """
    Interactive network topology map.
    Renders router at centre, devices as coloured nodes around it.
    Supports: zoom (wheel), pan (middle-click drag), node click, export PNG.
    """
    node_selected = pyqtSignal(str)   # emits IP of selected node
    ...
    # ── Public API ────────────────────────────────────────────────────────
    def load_devices(self, devices: list, local_ip: str = ""):
        """Build node list from discovered_devices."""
        self.nodes.clear()
        if not devices:
            self.update()
            return

        # Determine gateway (first .1 address in subnet)
        router_ip = ""
        if devices:
            parts = devices[0]["ip"].split(".")
            router_ip = f"{parts[0]}.{parts[1]}.{parts[2]}.1"

        # Always add a router node at centre
        ...
        # Spread hosts in a circle around the router
        host_devices = [d for d in devices if d["ip"] != router_ip]
        n = max(len(host_devices), 1)
        radius_base = 160 + n * 8          # scales with device count
        radius_base = min(radius_base, 340)

        for i, dev in enumerate(host_devices):
            angle = (2 * _math.pi * i / n) - _math.pi / 2
            # Multi-ring layout for large networks
            ring = 1 + (i // 16)
            r    = radius_base * ring
            node = TopologyNode(
                ip=dev["ip"],
                label=dev.get("hostname", dev["ip"]),
                status=dev.get("status", "ONLINE"),
                vendor=dev.get("vendor", ""),
                node_type="host"
            )
            node.x = r * _math.cos(angle)
            node.y = r * _math.sin(angle)
            self.nodes.append(node)

        self._reset_view()
        self.update()

    def update_node_status(self, ip: str, status: str):
        """Refresh a single node status (called by uptime monitor)."""
    ...
    def export_png(self, path: str):
        """Render topology to PNG file."""
        px = QPixmap(self.size())
        px.fill(QColor(self._BG))
        painter = QPainter(px)
    ...
    def wheelEvent(self, event):
        ...
        self._scale   *= factor
        self._scale    = max(0.15, min(self._scale, 5.0))
        self.update()
    ...
    def mousePressEvent(self, event):
        if event.button() == Qt.MouseButton.MiddleButton:
            ...
            if node:
                node.selected = True
                self.node_selected.emit(node.ip)
            self.update()
    ...
    # ── Hit test ──────────────────────────────────────────────────────────
    def _node_at(self, px, py) -> "TopologyNode | None":
        for node in reversed(self.nodes):
            nx = node.x * self._scale + self._offset_x
            ny = node.y * self._scale + self._offset_y
            if _math.hypot(px - nx, py - ny) <= TopologyNode.RADIUS * self._scale + 4:
                return node
        return None
    ...
    def _render(self, painter: QPainter):
        ...
        # ── Grid dots ──
        grid_spacing = max(30, int(40 * sc))
        painter.setPen(QPen(QColor(self._GRID), 1))
        gx0 = int(ox % grid_spacing)
        gy0 = int(oy % grid_spacing)
        for gx in range(gx0, W, grid_spacing):
            for gy in range(gy0, H, grid_spacing):
        ...
        if not self.nodes:
            ...
            painter.drawText(
                ...
                "◈  Run Device Discovery to render the topology map  ◈"
            )
            return

        router = next((n for n in self.nodes if n.node_type == "router"), None)

        # ── Edges (router → each host) ──
        painter.setOpacity(0.35)
        for node in self.nodes:
            if node is router or router is None:
                continue
            rx = router.x * sc + ox
            ry = router.y * sc + oy
            nx = node.x * sc + ox
            ny = node.y * sc + oy
            fg_hex, _ = node.color_pair()
            edge_color = QColor(fg_hex)
            # Pulsing dashed for offline
            pen = QPen(edge_color, max(1, sc * 1.2))
            if node.status == "OFFLINE":
                pen.setStyle(Qt.PenStyle.DashLine)
            else:
                pen.setStyle(Qt.PenStyle.SolidLine)
            painter.setPen(pen)
            painter.drawLine(int(rx), int(ry), int(nx), int(ny))
        ...
        for node in self.nodes:
            nx = node.x * sc + ox
            ny = node.y * sc + oy
            r  = TopologyNode.RADIUS * sc
            fg_hex, bg_hex = node.color_pair()

            # Glow ring for selected/hovered
            if node.selected:
                glow_pen = QPen(QColor(fg_hex), max(3, sc * 3))
                glow_pen.setStyle(Qt.PenStyle.SolidLine)
                painter.setPen(glow_pen)
                painter.setBrush(Qt.BrushStyle.NoBrush)
                painter.setOpacity(0.4)
                painter.drawEllipse(QRectF(nx - r - 6, ny - r - 6, (r + 6) * 2, (r + 6) * 2))
                painter.setOpacity(1.0)
            elif node.hovered:
                painter.setOpacity(0.3)
                painter.setPen(Qt.PenStyle.NoPen)
                painter.setBrush(QBrush(QColor(fg_hex)))
                painter.drawEllipse(QRectF(nx - r - 8, ny - r - 8, (r + 8) * 2, (r + 8) * 2))
                painter.setOpacity(1.0)

            # Radial gradient fill
            grad = QRadialGradient(nx - r * 0.3, ny - r * 0.3, r * 1.5)
            c1 = QColor(fg_hex)
            c1.setAlpha(220)
            c2 = QColor(bg_hex)
            c2.setAlpha(240)
            ...
            # Border
            border_pen = QPen(QColor(fg_hex), max(1.5, sc * 2))
            if node.selected:
                border_pen.setWidth(max(3, int(sc * 3)))
            painter.setPen(border_pen)
            painter.drawEllipse(QRectF(nx - r, ny - r, r * 2, r * 2))

            # Icon char inside node
            icon = {"router": "⬡", "gateway": "◈", "host": "●"}.get(node.node_type, "●")
            icon_font = QFont("Consolas", max(8, int(14 * sc)))
            ...
            dot_r = max(5, int(8 * sc))
            dot_x = nx + r * 0.65
            dot_y = ny + r * 0.65
            dot_clr = {"ONLINE": "#22c55e", "OFFLINE": "#ef4444",
                       "UNSTABLE": "#f59e0b", "ROUTER": "#00e5ff", "GATEWAY": "#7c3aed"}
            dot_color = dot_clr.get(node.status, "#64748b")
            painter.setPen(QPen(QColor("#0a0e17"), 2))
            painter.setBrush(QBrush(QColor(dot_color)))
            painter.drawEllipse(QRectF(dot_x - dot_r / 2, dot_y - dot_r / 2, dot_r, dot_r))

            # Label below node
            if sc > 0.4:
                lbl_font_sz = max(7, int(10 * sc))
                painter.setFont(QFont("Consolas", lbl_font_sz))
                label_text = node.ip if node.node_type == "router" else (
                    node.label if node.label != node.ip else node.ip
                )[:18]
                painter.setPen(QPen(QColor(self._LABEL), 1))
                painter.drawText(
                    QRectF(nx - 60, ny + r + 2, 120, 18),
                    Qt.AlignmentFlag.AlignCenter | Qt.AlignmentFlag.AlignTop,
                    label_text
                )
            # Vendor sub-label (only when zoomed in enough)
            if sc > 0.75 and node.vendor:
                painter.setFont(QFont("Consolas", max(6, int(8 * sc))))
                painter.setPen(QPen(QColor(self._LABEL2), 1))
                painter.drawText(
                    QRectF(nx - 70, ny + r + 16, 140, 14),
                    Qt.AlignmentFlag.AlignCenter | Qt.AlignmentFlag.AlignTop,
                    node.vendor[:20]
                )
        ...
        painter.setPen(QPen(QColor("#1e3a5f"), 1))
        painter.drawText(QRectF(8, H - 22, 350, 18),
                         Qt.AlignmentFlag.AlignLeft | Qt.AlignmentFlag.AlignVCenter,
                         "Scroll: zoom  │  Middle-drag: pan  │  F: fit  │  Click node: select/scan")

    def _draw_legend(self, painter: QPainter, W: int, H: int):
        legend_items = [
            ("#00e5ff", "Router / Gateway"),
            ("#22c55e", "Online"),
            ("#f59e0b", "Unstable"),
            ("#ef4444", "Offline"),
        ]
        ...
        for i, (clr, lbl) in enumerate(legend_items):
            ...
            painter.drawEllipse(QRectF(lx, cy + 3, 10, 10))
            painter.setPen(QPen(QColor("#64748b"), 1))
            painter.drawText(int(lx + 16), int(cy + 13), lbl)


# ═══════════════════════════════════════════════════════════════════════════════
#  REAL-TIME TRAFFIC DASHBOARD  (v10.0)
#  Live scrolling charts — packets/sec, protocol breakdown, top talkers
# ═══════════════════════════════════════════════════════════════════════════════
class TrafficDashboard(QWidget):
    """
    Live traffic charts powered by matplotlib.
    - Packets/sec scrolling line graph (60-second window)
    - Protocol breakdown bar chart (live)
    - Bandwidth (bytes/sec) line graph
    - Top-talkers table
    Falls back to a plain-text stats panel if matplotlib is unavailable.
    """

    def __init__(self, parent=None):
        super().__init__(parent)
        self._history_len = 60           # seconds of rolling window
        self._pps_history  = [0] * self._history_len
        self._bps_history  = [0] * self._history_len
        self._last_tick_total  = 0
        self._last_tick_bytes  = 0
        self._proto_counts = {"TCP": 0, "UDP": 0, "ICMP": 0, "Other": 0}
        self._talkers: dict[str, int] = {}  # ip → packet count
        self._running = False
        ...
        if _MPL_AVAILABLE:
            self._build_mpl_ui(layout)
        else:
            self._build_fallback_ui(layout)

        # Tick timer — updates charts every second
        self._timer = QTimer(self)
        self._timer.setInterval(1000)
        self._timer.timeout.connect(self._tick)

    # ── UI builders ───────────────────────────────────────────────────────
    def _build_mpl_ui(self, layout):
        ...
        # 2×2 figure
        self._fig = Figure(figsize=(12, 7), dpi=90, facecolor="#0a0e17")
        self._canvas = FigureCanvas(self._fig)
        self._canvas.setStyleSheet("background:#0a0e17;")
        layout.addWidget(self._canvas, 3)

        gs = self._fig.add_gridspec(2, 2, hspace=0.45, wspace=0.35,
                                     left=0.07, right=0.97, top=0.92, bottom=0.08)
        self._ax_pps  = self._fig.add_subplot(gs[0, 0])  # Packets/sec
        self._ax_bps  = self._fig.add_subplot(gs[0, 1])  # Bytes/sec
        self._ax_proto = self._fig.add_subplot(gs[1, 0])  # Protocol bars
        self._ax_talk  = self._fig.add_subplot(gs[1, 1])  # Top talkers

        for ax in (self._ax_pps, self._ax_bps, self._ax_proto, self._ax_talk):
            ax.set_facecolor("#0d1321")
            ax.tick_params(colors="#64748b", labelsize=7)
            ax.spines["bottom"].set_color("#1e2d42")
            ax.spines["left"].set_color("#1e2d42")
            ax.spines["top"].set_visible(False)
            ax.spines["right"].set_visible(False)

        # Titles
        for ax, title in [
            (self._ax_pps,   "Packets / sec  (last 60s)"),
            (self._ax_bps,   "Bandwidth  (bytes/sec, last 60s)"),
            (self._ax_proto, "Protocol Breakdown"),
            (self._ax_talk,  "Top Talkers  (by packets)"),
        ]:
            ax.set_title(title, color="#00e5ff", fontsize=9, pad=5, fontfamily="monospace")

        self._x_axis = list(range(-self._history_len + 1, 1))

        # Talkers table (below figure)
        self._talker_table = QTableWidget(0, 3)
        self._talker_table.setHorizontalHeaderLabels(["IP ADDRESS", "PACKETS", "BANDWIDTH"])
        self._talker_table.horizontalHeader().setSectionResizeMode(0, QHeaderView.ResizeMode.Stretch)
        self._talker_table.setColumnWidth(1, 90)
        self._talker_table.setColumnWidth(2, 110)
        self._talker_table.setMaximumHeight(160)
        self._talker_table.setEditTriggers(QTableWidget.EditTrigger.NoEditTriggers)
        self._talker_table.setAlternatingRowColors(True)
        layout.addWidget(self._talker_table, 1)

    def _build_fallback_ui(self, layout):
        """Plain text stats when matplotlib is not installed."""
        msg = QLabel(
            "\U0001F4E6  Install matplotlib & numpy for live traffic charts:\n\n""    pip install matplotlib numpy\n\nThen restart SIRA.\n\n""Live stats are still shown below while capture runs."
        )
        msg.setStyleSheet(f"color:#64748b; font-size:13px; padding:20px;")
        msg.setAlignment(Qt.AlignmentFlag.AlignCenter)
        layout.addWidget(msg)
        self._fallback_box = QTextEdit()
        self._fallback_box.setReadOnly(True)
        self._fallback_box.setFont(QFont("Consolas", 11))
        layout.addWidget(self._fallback_box, 1)
    ...
    def feed_packet(self, pkt: dict):
        """Called for every captured packet."""
        proto = pkt.get("proto", "Other")
        self._proto_counts[proto if proto in self._proto_counts else "Other"] += 1
        # Track talkers by source IP
        src = pkt.get("src_ip", "")
        if src:
            self._talkers[src] = self._talkers.get(src, 0) + 1

    def feed_stats(self, stats: dict):
        """Called on each stats_update signal from PacketCaptureWorker."""
        self._proto_counts["TCP"]   = stats.get("TCP", 0)
        self._proto_counts["UDP"]   = stats.get("UDP", 0)
        self._proto_counts["ICMP"]  = stats.get("ICMP", 0)
        self._proto_counts["Other"] = stats.get("Other", 0)

    # ── Tick ─────────────────────────────────────────────────────────────
    def _tick(self):
        total = sum(self._proto_counts.values())
        bytez = sum(self._talkers.values()) * 150   # rough estimate

        pps = total - self._last_tick_total
        bps = bytez - self._last_tick_bytes
        self._last_tick_total = total
        self._last_tick_bytes = bytez

        self._pps_history.append(max(0, pps))
        self._bps_history.append(max(0, bps))
        if len(self._pps_history) > self._history_len:
            self._pps_history.pop(0)
        if len(self._bps_history) > self._history_len:
            self._bps_history.pop(0)

        if _MPL_AVAILABLE:
            self._redraw_mpl()
        else:
            self._redraw_fallback(total, pps, bps)

    def _redraw_mpl(self):
        import numpy as _np
        ...
        ax.cla()
        ax.set_facecolor("#0d1321")
        ax.set_title("Packets / sec  (last 60s)", color="#00e5ff", fontsize=9, pad=5, fontfamily="monospace")
        ax.tick_params(colors="#64748b", labelsize=7)
        for sp in ax.spines.values():
            sp.set_color("#1e2d42")
        ax.spines["top"].set_visible(False)
        ax.spines["right"].set_visible(False)
        pps_arr = self._pps_history[-self._history_len:]
        ax.fill_between(x[:len(pps_arr)], pps_arr, alpha=0.25, color="#00e5ff")
        ax.plot(x[:len(pps_arr)], pps_arr, color="#00e5ff", linewidth=1.5)
        ax.set_xlim(-self._history_len, 0)
        ax.set_ylim(bottom=0)
        ax.set_ylabel("pkt/s", color="#64748b", fontsize=7)
        ax.set_xlabel("seconds ago", color="#64748b", fontsize=7)
        if max(pps_arr, default=0) > 0:
            ax.axhline(y=_np.mean(pps_arr), color="#f59e0b", linewidth=0.8,
                       linestyle="--", alpha=0.6, label=f"avg {_np.mean(pps_arr):.1f}")
        ...
        ax2.set_title("Bandwidth  (bytes/sec, last 60s)", color="#00e5ff", fontsize=9, pad=5, fontfamily="monospace")
        ax2.tick_params(colors="#64748b", labelsize=7)
        for sp in ax2.spines.values():
            sp.set_color("#1e2d42")
        ax2.spines["top"].set_visible(False)
        ax2.spines["right"].set_visible(False)
        bps_arr = self._bps_history[-self._history_len:]
        ax2.fill_between(x[:len(bps_arr)], bps_arr, alpha=0.25, color="#22c55e")
        ax2.plot(x[:len(bps_arr)], bps_arr, color="#22c55e", linewidth=1.5)
        ax2.set_xlim(-self._history_len, 0)
        ax2.set_ylim(bottom=0)
        ax2.set_ylabel("B/s", color="#64748b", fontsize=7)
        ax2.set_xlabel("seconds ago", color="#64748b", fontsize=7)

        # ── Protocol breakdown ──
        ax3 = self._ax_proto
        ax3.cla()
        ax3.set_facecolor("#0d1321")
        ax3.set_title("Protocol Breakdown", color="#00e5ff", fontsize=9, pad=5, fontfamily="monospace")
        ax3.tick_params(colors="#64748b", labelsize=7)
        for sp in ax3.spines.values():
            sp.set_color("#1e2d42")
        ax3.spines["top"].set_visible(False)
        ax3.spines["right"].set_visible(False)
        protos = list(self._proto_counts.keys())
        counts = [self._proto_counts[p] for p in protos]
        bar_colors = ["#00e5ff", "#22c55e", "#f59e0b", "#64748b"]
        bars = ax3.bar(protos, counts, color=bar_colors[:len(protos)], width=0.5)
        ax3.set_ylabel("packets", color="#64748b", fontsize=7)
        ...
        ax4.set_title("Top Talkers  (by packets)", color="#00e5ff", fontsize=9, pad=5, fontfamily="monospace")
        ...
        if self._talkers:
            top = sorted(self._talkers.items(), key=lambda x: -x[1])[:8]
            t_ips   = [t[0] for t in top]
            t_cnts  = [t[1] for t in top]
            t_clrs  = ["#ef4444" if i == 0 else "#f97316" if i == 1 else "#eab308" if i == 2
                       else "#64748b" for i in range(len(top))]
            ax4.barh(t_ips, t_cnts, color=t_clrs, height=0.5)
            ax4.set_xlabel("packets", color="#64748b", fontsize=7)
            ax4.invert_yaxis()
            for i, (ip, cnt) in enumerate(zip(t_ips, t_cnts)):
                ax4.text(cnt + max(t_cnts) * 0.01, i, str(cnt),
                         va="center", color="#e2e8f0", fontsize=7)
        else:
            ax4.text(0.5, 0.5, "No traffic captured yet",
        ...
        # ── Talker table ──
        top_n = sorted(self._talkers.items(), key=lambda x: -x[1])[:15]
        self._talker_table.setRowCount(len(top_n))
        total_pkts = max(sum(self._talkers.values()), 1)
        for row, (ip, pkts) in enumerate(top_n):
            clr = "#ef4444" if row == 0 else "#f97316" if row == 1 else                   "#eab308" if row == 2 else "#e2e8f0"
            pct = pkts / total_pkts * 100
            est_bytes = pkts * 150
            for col, txt in enumerate([ip, f"{pkts:,}  ({pct:.1f}%)", self._human_bytes(est_bytes)]):
                item = QTableWidgetItem(txt)
                item.setForeground(QColor(clr))
                self._talker_table.setItem(row, col, item)

    def _redraw_fallback(self, total, pps, bps):
        box = self._fallback_box
        box.clear()
        def w(txt, clr="#e2e8f0"):
            box.setTextColor(QColor(clr))
            box.append(txt)
        w("═" * 55, "#00e5ff")
        w("  LIVE TRAFFIC STATISTICS", "#00e5ff")
        w("═" * 55, "#00e5ff")
        w(f"  Total packets : {total:,}", "#e2e8f0")
        w(f"  Packets/sec   : {pps:,}", "#22c55e" if pps > 0 else "#64748b")
        w("")
        w("  Protocol Breakdown:", "#00e5ff")
        for proto, cnt in self._proto_counts.items():
            pct = cnt / max(total, 1) * 100
            bar = "█" * int(30 * pct / 100) + "░" * (30 - int(30 * pct / 100))
            clr = {"TCP":"#00e5ff","UDP":"#22c55e","ICMP":"#f59e0b"}.get(proto,"#64748b")
            w(f"  {proto:<6}  [{bar}]  {cnt:,}  ({pct:.1f}%)", clr)
        w("")
        w("  Top Talkers:", "#00e5ff")
        for ip, pkts in sorted(self._talkers.items(), key=lambda x: -x[1])[:10]:
            w(f"  {ip:<18}  {pkts:>6} pkts", "#e2e8f0")
        w("═" * 55, "#00e5ff")
...
class SIRA(QMainWindow):
    def __init__(self):
        ...
        self.device_row_map   = {}
        self.settings         = QSettings("SIRA", "Settings")

        self.selected_ports    = list(range(1, 1001))
        self.ports_display_str = "1-1000"

        self.setWindowTitle("SIRA v10.0 — Security Intelligence & Recon Agent  |  Network · Vulnerabilities · Threat Intel")
        ...
        self._refresh_history_list()   # Load scan history from DB
    ...
    def _create_toolbar(self):
        ...
        for label, slot, tip in [
            ...
            ("📄  PDF Report",  self.export_pdf_report,    "Export professional PDF report"),
            ("🎯  Risk Score",  self.show_risk_score,      "Calculate & display risk score"),
            ("🌍  GeoIP",       self.quick_geoip,          "GeoIP lookup for current target"),
            ("🗺  Topology",    self._render_topology,     "Render network topology map"),
            ("📊  Traffic",     self._show_traffic_tab,    "Open traffic dashboard"),
    ...
    def _make_header(self):
        ...
        subtitle = QLabel("Security Intelligence &amp; Recon Agent  ·  v10.0")
    ...
    def _make_sidebar(self):
        ...
        """)
        self.target_input.textChanged.connect(lambda t: self.geo_input.setText(t) if hasattr(self, "geo_input") else None)
        self.target_input.textChanged.connect(lambda t: self.tr_target_input.setText(t) if hasattr(self, "tr_target_input") else None)
    ...
    def _make_main_area(self):
        ...
        # ── RISK SCORE TAB (v10.0) ──────────────────────────────────────────
        risk_widget = QWidget()
        risk_layout = QVBoxLayout(risk_widget)
        risk_layout.setContentsMargins(8, 8, 8, 8)
        risk_layout.setSpacing(8)
        self.risk_box = QTextEdit()
        self.risk_box.setReadOnly(True)
        self.risk_box.setFont(QFont("Consolas", 11))
        risk_layout.addWidget(self.risk_box)
        self.tabs.addTab(risk_widget, "🎯 Risk Score")

        # ── SCAN HISTORY & DIFF TAB (v10.0) ─────────────────────────────────
        hist_widget = QWidget()
        ...
        self.hist_list.setMaximumHeight(130)
        self.hist_list.setSelectionMode(QAbstractItemView.SelectionMode.SingleSelection)
        self.hist_diff_btn = QPushButton("🔀  DIFF vs Previous")
        self.hist_diff_btn.clicked.connect(self.run_scan_diff)
        self.hist_clear_btn = QPushButton("🗑  Clear History")
        self.hist_clear_btn.clicked.connect(self.clear_scan_history)
        hist_ctrl.addWidget(self.hist_diff_btn)
        hist_ctrl.addWidget(self.hist_clear_btn)
        hist_ctrl.addStretch()
        hist_layout.addWidget(QLabel("  Scan History (click to view, select two for diff):"))
        hist_layout.addWidget(self.hist_list)
        hist_layout.addLayout(hist_ctrl)
        self.hist_box = QTextEdit()
        self.hist_box.setReadOnly(True)
        self.hist_box.setFont(QFont("Consolas", 11))
        hist_layout.addWidget(self.hist_box, 1)
        self.hist_list.itemClicked.connect(self.load_history_item)
        self.tabs.addTab(hist_widget, "📂 History")

        # ── GEOIP INTELLIGENCE TAB (v10.0) ──────────────────────────────────
        geo_widget = QWidget()
        ...
        self.geo_input.setPlaceholderText("IP address or hostname to geo-locate")
        self.geo_btn = QPushButton("🌍  LOOKUP")
        self.geo_btn.setObjectName("discoverBtn")
        self.geo_btn.clicked.connect(self.run_geoip_lookup)
        geo_ctrl.addWidget(QLabel("Target:"))
        geo_ctrl.addWidget(self.geo_input, 1)
        geo_ctrl.addWidget(self.geo_btn)
        geo_layout.addLayout(geo_ctrl)
        self.geo_box = QTextEdit()
        self.geo_box.setReadOnly(True)
        self.geo_box.setFont(QFont("Consolas", 11))
        geo_layout.addWidget(self.geo_box, 1)
        self.tabs.addTab(geo_widget, "🌍 GeoIP")

        # ── AI THREAT BRIEF TAB (v10.0) ──────────────────────────────────────
        ai_widget = QWidget()
        ai_layout = QVBoxLayout(ai_widget)
        ai_layout.setContentsMargins(8, 8, 8, 8)
        ai_layout.setSpacing(6)
        ai_ctrl = QHBoxLayout()
        self.ai_btn = QPushButton("🤖  GENERATE AI THREAT BRIEF")
        self.ai_btn.setObjectName("scanBtn")
        self.ai_btn.setMinimumHeight(36)
        self.ai_btn.clicked.connect(self.generate_ai_threat_brief)
        ai_ctrl.addWidget(self.ai_btn)
        ai_ctrl.addStretch()
        ai_label = QLabel("Uses Anthropic Claude API — requires ANTHROPIC_API_KEY env var")
        ai_label.setStyleSheet(f"color:{TEXT_SECONDARY}; font-size:11px;")
        ai_ctrl.addWidget(ai_label)
        ai_layout.addLayout(ai_ctrl)
        self.ai_box = QTextEdit()
        self.ai_box.setReadOnly(True)
        self.ai_box.setFont(QFont("Consolas", 11))
        ai_layout.addWidget(self.ai_box, 1)
        self.tabs.addTab(ai_widget, "🤖 AI Brief")

        # ── NETWORK TOPOLOGY MAP TAB (v10.0) ────────────────────────────────
        topo_widget = QWidget()
        topo_layout = QVBoxLayout(topo_widget)
        topo_layout.setContentsMargins(6, 6, 6, 6)
        topo_layout.setSpacing(4)

        topo_ctrl = QHBoxLayout()
        topo_ctrl.setSpacing(6)
        self.topo_refresh_btn = QPushButton("🔄  RENDER TOPOLOGY")
        self.topo_refresh_btn.setObjectName("discoverBtn")
        self.topo_refresh_btn.setFixedHeight(32)
        self.topo_refresh_btn.clicked.connect(self._render_topology)
        self.topo_fit_btn = QPushButton("⊡  FIT VIEW  [F]")
        self.topo_fit_btn.setFixedHeight(32)
        self.topo_fit_btn.clicked.connect(self._topo_fit)
        self.topo_export_btn = QPushButton("📷  EXPORT PNG")
        self.topo_export_btn.setFixedHeight(32)
        self.topo_export_btn.clicked.connect(self._topo_export_png)
        self.topo_info_lbl = QLabel("  No topology — run device discovery first, then click Render")
        self.topo_info_lbl.setStyleSheet(f"color:{TEXT_SECONDARY}; font-size:11px;")
        for w_ in [self.topo_refresh_btn, self.topo_fit_btn, self.topo_export_btn, self.topo_info_lbl]:
            topo_ctrl.addWidget(w_)
        topo_ctrl.addStretch()
        topo_layout.addLayout(topo_ctrl)

        self.topo_canvas = NetworkTopologyCanvas(self)
        self.topo_canvas.node_selected.connect(self._topo_node_selected)
        topo_layout.addWidget(self.topo_canvas, 1)
        self.tabs.addTab(topo_widget, "🗺 Topology")

        # ── REAL-TIME TRAFFIC DASHBOARD TAB (v10.0) ─────────────────────────
        self.traffic_dashboard = TrafficDashboard(self)
        self.tabs.addTab(self.traffic_dashboard, "📊 Traffic")
    ...
    def on_discovery_done(self, devices: list):
        ...
        self.status_dot.setStyleSheet(f"color:{LOW}; font-size:12px; font-weight:700; padding:0 10px;")
        # Auto-render topology after discovery
    ...
    def update_device_status(self, update: dict):
        ...
        # Live update topology node colour
        if hasattr(self, "topo_canvas"):
            self.topo_canvas.update_node_status(ip, status)
    ...
    def _build_summary(self, r):
        ...
        w("═" * 72, ACCENT)
        w("  SIRA v10.0 — SCAN SUMMARY", ACCENT)
        w("═" * 72, ACCENT)
    ...
    def _build_statistics(self):
        ...
        w("═" * 72, ACCENT)
        w("  SIRA v10.0 — SCAN STATISTICS & METRICS", ACCENT)
        w("═" * 72, ACCENT)
    ...
    def toggle_packet_capture(self):
        if self.capture_worker and self.capture_worker.isRunning():
            ...
            self.capture_worker.stats_update.connect(self.traffic_dashboard.feed_stats)
            self.capture_worker.log.connect(self.append_log)
            self.capture_worker.start()
            self.traffic_dashboard.start()
    ...
    def add_packet_row(self, pkt: dict):
        ...
        self._add_packet_row_ui(pkt)
        # Feed into traffic dashboard
        if hasattr(self, "traffic_dashboard"):
            self.traffic_dashboard.feed_packet(pkt)
...
{dev_rows or '<tr><td colspan=7>No discovery run</td></tr>'}</table>
    ...
    def _export_text(self, path):
        ...
        with open(path, "w", encoding="utf-8") as f:
            f.write("SIRA v10.0 — SCAN REPORT\n" + "=" * 60 + "\n")
            r = self.results
    ...
    def auto_export_report(self):
        try:
            from pathlib import Path
            import tempfile
            temp_dir = Path(tempfile.gettempdir()) / "SIRA"
    ...
    # ══════════════════════════════════════════════════════════════════════
    #  TOPOLOGY MAP METHODS  (v10.0)
    # ══════════════════════════════════════════════════════════════════════

    def _render_topology(self):
        """Build topology from discovered_devices and draw it."""
        if not self.discovered_devices:
            self.append_log("[!] Run device discovery first to build the topology map.", "warning")
            if hasattr(self, "topo_info_lbl"):
                self.topo_info_lbl.setText("  No topology — run device discovery first, then click Render")
            return
        local_ip = get_local_ip()
        self.topo_canvas.load_devices(self.discovered_devices, local_ip)
        n = len(self.discovered_devices)
        self.topo_info_lbl.setText(
            f"  {n} device(s)  ·  scroll=zoom  ·  middle-drag=pan  ·  click node=select  ·  F=fit"
        )
        for i in range(self.tabs.count()):
            if "Topology" in self.tabs.tabText(i):
                self.tabs.setCurrentIndex(i)
                break
        self.append_log(f"[✓] Topology map rendered — {n} device(s).", "success")

    def _topo_fit(self):
        self.topo_canvas._reset_view()
        self.topo_canvas.update()

    def _topo_export_png(self):
        path, _ = QFileDialog.getSaveFileName(
            self, "Export Topology PNG",
            f"SIRA_topology_{datetime.now().strftime('%Y%m%d_%H%M%S')}.png",
            "PNG Image (*.png)"
        )
        if not path:
            return
        try:
            self.topo_canvas.export_png(path)
            self.append_log(f"[✓] Topology exported: {path}", "success")
            QMessageBox.information(self, "Exported", f"Topology map saved to:\n{path}")
        except Exception as e:
            self.append_log(f"[-] Topology export error: {e}", "error")

    def _show_traffic_tab(self):
        """Switch to the traffic dashboard tab."""
        for i in range(self.tabs.count()):
            if "Traffic" in self.tabs.tabText(i):
                self.tabs.setCurrentIndex(i)
                if not (self.capture_worker and self.capture_worker.isRunning()):
                    self.append_log("[!] Start packet capture first to see live traffic data.", "warning")
                return

    def _topo_node_selected(self, ip: str):
        self.target_input.setText(ip)
        self.append_log(f"[✓] Topology: node {ip} selected — set as scan target.", "success")
        node = next((n for n in self.topo_canvas.nodes if n.ip == ip), None)
        if node and hasattr(self, "topo_info_lbl"):
            self.topo_info_lbl.setText(
                f"  Selected: {ip}  ({node.label})  Status: {node.status}  Vendor: {node.vendor}"
            )
    ...
    # ──────────────────────────────────────────
    def show_help(self):
        QMessageBox.information(self, "SIRA v10.0 Help", """
<h2>SIRA v10.0 — Security Intelligence &amp; Recon Agent</h2>

<h3>🎯 Target Configuration</h3>
<p>Enter an IP, hostname, or range. Click <b>localhost</b> or <b>Local Net</b> for quick fills.</p>

<h3>🔌 Port Selection</h3>
<p>Click <b>"Configure Ports …"</b> or use quick buttons (Top 100, Web, All, DB).</p>

<h3>🎯 Risk Score (v10.0)</h3>
<p>After a scan, click <b>Risk Score</b> in the toolbar — 0-100 threat gauge weighted by CVE severity, port danger, and exposure.</p>

<h3>📂 Scan History &amp; Diff (v10.0)</h3>
<p>Every scan saves to SQLite. Go to the <b>History</b> tab, click <b>DIFF vs Previous</b> to see changes.</p>

<h3>🌍 GeoIP Intelligence (v10.0)</h3>
<p>Use the <b>GeoIP</b> tab for country/city/ISP/AS lookup on any IP.</p>

<h3>📄 PDF Report (v10.0)</h3>
<p>Click <b>PDF Report</b> in the toolbar. Requires: <code>pip install reportlab</code></p>

<h3>🤖 AI Threat Brief (v10.0)</h3>
<p>Set <code>ANTHROPIC_API_KEY</code> env var, then use the <b>AI Brief</b> tab for Claude-powered threat analysis.</p>

<h3>🦈 Packet Capture</h3>
<p>Full Wireshark-level capture with protocol dissector, hex dump, display filters, Follow TCP Stream, and real .pcap export.</p>

<h3>💾 Export</h3>
<p>Reports available as JSON / HTML / TXT / PDF.</p>
""")
    ...
    #  RISK SCORE ENGINE
    # ──────────────────────────────────────────
    def _calculate_risk_score(self, results: dict = None) -> dict:
        """
        Calculate a 0-100 risk score from scan results.
        Weights: CVE severity (50%), open port risk (30%), service exposure (20%).
        """
        ...
        # CVE score (50 pts max)
        cve_pts = 0
        for v in vulns:
            sev = v.get("severity", "").upper()
            score_map = {"CRITICAL": 20, "HIGH": 10, "MEDIUM": 5, "LOW": 2}
            cve_pts += score_map.get(sev, 1)
            if v.get("exploit_available"):
                cve_pts += 5  # exploit known = bonus risk
        cve_pts = min(cve_pts, 50)

        # Port risk score (30 pts max)
        HIGH_RISK_PORTS = {
            23: 10,   # Telnet (plaintext)
            445: 10,  # SMB (ransomware target)
            21: 8,    # FTP (plaintext)
            3389: 8,  # RDP (brute force target)
            5900: 7,  # VNC
            1433: 6,  # MSSQL
            3306: 6,  # MySQL exposed
            27017: 8, # MongoDB exposed
            6379: 8,  # Redis exposed (no auth by default)
            9200: 6,  # Elasticsearch
        }
        port_pts = 0
        for p in ports:
            port_pts += HIGH_RISK_PORTS.get(p["port"], 1)
        port_pts = min(port_pts, 30)

        # Service exposure (20 pts max)
        exposure_pts = min(len(ports) * 2, 20)

        total = cve_pts + port_pts + exposure_pts
        total = min(total, 100)

        # Grade
        if total >= 75:   grade = "CRITICAL"
        elif total >= 50: grade = "HIGH"
        elif total >= 25: grade = "MEDIUM"
        elif total >= 10: grade = "LOW"
        else:             grade = "MINIMAL"
        ...
        return {
            ...
            "critical_cves": sum(1 for v in vulns if v.get("severity","").upper()=="CRITICAL"),
            "exploitable":   sum(1 for v in vulns if v.get("exploit_available")),
        }

    def show_risk_score(self):
        """Build and display the risk score panel."""
        if not self.results:
            self.tabs.setCurrentIndex(self.tabs.indexOf(self.risk_box.parent().parent()))
            self.risk_box.clear()
            self.risk_box.setTextColor(QColor(TEXT_SECONDARY))
            self.risk_box.append("\n  No scan results available. Run a scan first.")
            return

        rs = self._calculate_risk_score()
        box = self.risk_box
        box.clear()

        grade_color = {
            "CRITICAL": CRITICAL, "HIGH": HIGH,
            "MEDIUM": MEDIUM, "LOW": LOW, "MINIMAL": SUCCESS, "N/A": TEXT_SECONDARY
        }.get(rs["grade"], TEXT_SECONDARY)

        def w(txt, clr=TEXT_PRIMARY):
            box.setTextColor(QColor(clr))
            box.append(txt)

        w("═" * 72, ACCENT)
        w("  SIRA v10.0 — RISK SCORE REPORT", ACCENT)
        w("═" * 72, ACCENT)
        w(f"  Target : {self.results.get('target','—')}  ({self.results.get('ip','—')})", TEXT_SECONDARY)
        w(f"  Scanned: {self.results.get('start_time','—')}", TEXT_SECONDARY)
        w("")

        # Big score gauge
        score = rs["score"]
        bar_full = 60
        filled = int(bar_full * score / 100)
        bar = "█" * filled + "░" * (bar_full - filled)
        w(f"  OVERALL RISK SCORE", ACCENT)
        w(f"  [{bar}]  {score}/100", grade_color)
        w(f"  GRADE: {rs['grade']}", grade_color)
        w("")

        # Breakdown
        w("  ── SCORE BREAKDOWN ──────────────────────────────────────────", ACCENT)
        for label, pts, max_pts, clr in [
            ("CVE Severity Weight    (max 50)", rs["cve_pts"],      50, CRITICAL),
            ("Open Port Risk         (max 30)", rs["port_pts"],      30, HIGH),
            ("Service Exposure       (max 20)", rs["exposure_pts"],  20, MEDIUM),
        ]:
            b2 = int(40 * pts / max_pts) if max_pts > 0 else 0
            bar2 = "█" * b2 + "░" * (40 - b2)
            w(f"  {label:<38} [{bar2}] {pts:>3}/{max_pts}", clr)
        w("")

        # Stats
        w("  ── THREAT INDICATORS ────────────────────────────────────────", ACCENT)
        w(f"  Total CVEs found        : {rs['total_cves']}", TEXT_PRIMARY)
        w(f"  Critical severity CVEs  : {rs['critical_cves']}", CRITICAL if rs["critical_cves"] > 0 else TEXT_SECONDARY)
        w(f"  CVEs with known exploit : {rs['exploitable']}", HIGH if rs["exploitable"] > 0 else TEXT_SECONDARY)
        w(f"  Open ports              : {rs['total_ports']}", TEXT_PRIMARY)
        w("")

        # Recommendations
        w("  ── RECOMMENDATIONS ──────────────────────────────────────────", ACCENT)
        recs = []
        if rs["critical_cves"] > 0:
            recs.append("🔴 URGENT: Patch all CRITICAL CVEs immediately — exploit code may be public")
        if rs["exploitable"] > 0:
            recs.append(f"🔴 {rs['exploitable']} CVE(s) have confirmed exploits — treat as actively exploited")
        if any(p["port"] in [23, 21] for p in self.results.get("open_ports", [])):
            recs.append("🟠 Disable Telnet/FTP — plaintext protocols leak credentials")
        if any(p["port"] == 445 for p in self.results.get("open_ports", [])):
            recs.append("🟠 SMB (445) is open — ensure patched against EternalBlue/WannaCry")
        if any(p["port"] in [27017, 6379, 9200] for p in self.results.get("open_ports", [])):
            recs.append("🟠 Database/cache service exposed — check if authentication is enabled")
        if not recs:
            recs.append("✅ No critical issues detected — maintain regular scan schedule")
        for rec in recs:
            w(f"  {rec}", WARNING if "🟠" in rec else (CRITICAL if "🔴" in rec else LOW))
        w("")
        w("═" * 72, ACCENT)

        # Navigate to risk tab
        for i in range(self.tabs.count()):
            if "Risk" in self.tabs.tabText(i):
                self.tabs.setCurrentIndex(i)
                break

    # ──────────────────────────────────────────
    #  SCAN HISTORY & DIFF (v10.0)
    # ──────────────────────────────────────────
    def _save_scan_to_history(self, results: dict):
        """Save scan result to SQLite history DB."""
        import sqlite3, json as _json
        db_path = Path.home() / "SIRA_logs" / "scan_history.db"
        db_path.parent.mkdir(exist_ok=True)
        conn = sqlite3.connect(str(db_path))
        conn.execute("""
            CREATE TABLE IF NOT EXISTS scans (
                id INTEGER PRIMARY KEY AUTOINCREMENT,
                ts TEXT, target TEXT, ip TEXT,
                open_ports INTEGER, cves INTEGER,
                grade TEXT, score INTEGER, data TEXT
            )
        """)
        rs = self._calculate_risk_score(results)
        conn.execute("""
            INSERT INTO scans (ts, target, ip, open_ports, cves, grade, score, data)
            VALUES (?,?,?,?,?,?,?,?)
        """, (
            datetime.now().isoformat(),
            results.get("target",""), results.get("ip",""),
            len(results.get("open_ports",[])),
            len(results.get("vulns",[])),
            rs["grade"], rs["score"],
            _json.dumps(results)
        ))
        conn.commit()
    ...
    def _refresh_history_list(self):
        """Reload history list widget from DB."""
        import sqlite3
        db_path = Path.home() / "SIRA_logs" / "scan_history.db"
        ...
        try:
            conn = sqlite3.connect(str(db_path))
            rows = conn.execute(
                "SELECT id, ts, target, ip, open_ports, cves, grade, score FROM scans ORDER BY id DESC LIMIT 50"
            ).fetchall()
            conn.close()
            self.hist_list.clear()
            for row in rows:
                rid, ts, target, ip, ports, cves, grade, score = row
                label = f"[{rid}] {ts[:16]}  {target}  •  {ports} ports  {cves} CVEs  {grade}({score})"
                item = QListWidgetItem(label)
                item.setData(Qt.ItemDataRole.UserRole, rid)
                clr = {
                    "CRITICAL": CRITICAL, "HIGH": HIGH,
                    "MEDIUM": MEDIUM, "LOW": LOW, "MINIMAL": SUCCESS
                }.get(grade, TEXT_SECONDARY)
                item.setForeground(QColor(clr))
                self.hist_list.addItem(item)
        except Exception as e:
            self.append_log(f"[-] History load error: {e}", "error")

    def load_history_item(self, item):
        """Load a historical scan result into history view."""
        import sqlite3, json as _json
        ...
        try:
            conn = sqlite3.connect(str(db_path))
            row = conn.execute("SELECT data FROM scans WHERE id=?", (rid,)).fetchone()
            conn.close()
            if not row:
                return
            ...
            grade_clr = {"CRITICAL":CRITICAL,"HIGH":HIGH,"MEDIUM":MEDIUM,"LOW":LOW,"MINIMAL":SUCCESS}.get(rs["grade"],TEXT_SECONDARY)
            w("═" * 70, ACCENT)
            w(f"  HISTORICAL SCAN  —  Record #{rid}", ACCENT)
            w("═" * 70, ACCENT)
            w(f"  Target     : {results.get('target','—')}  ({results.get('ip','—')})", TEXT_SECONDARY)
            w(f"  Scanned    : {results.get('start_time','—')}  →  {results.get('end_time','—')}", TEXT_SECONDARY)
            w(f"  Duration   : {results.get('scan_duration',0)}s", TEXT_SECONDARY)
            w(f"  Risk Score : {rs['score']}/100  [{rs['grade']}]", grade_clr)
            w("")
            w(f"  Open Ports ({len(results.get('open_ports',[]))}):", ACCENT)
            for p in results.get("open_ports", []):
                w(f"    {p['port']}/{p.get('proto','tcp')}  {p['service']}  v{p.get('version','?')}", LOW)
            w("")
            w(f"  CVEs ({len(results.get('vulns',[]))}):", ACCENT)
            for v in results.get("vulns", []):
                clr2 = {"CRITICAL":CRITICAL,"HIGH":HIGH,"MEDIUM":MEDIUM,"LOW":LOW}.get(v["severity"].upper(),TEXT_SECONDARY)
                w(f"    {v['id']}  [{v['severity']}]  {v['service']}:{v['port']}", clr2)
        except Exception as e:
            self.append_log(f"[-] History load error: {e}", "error")

    def run_scan_diff(self):
        """Diff the two most recent scans in history."""
        import sqlite3, json as _json
        db_path = Path.home() / "SIRA_logs" / "scan_history.db"
        if not db_path.exists():
            self.hist_box.append("\n  No history found. Run at least two scans first.")
            return
        try:
            conn = sqlite3.connect(str(db_path))
            rows = conn.execute(
                "SELECT id, ts, target, data FROM scans ORDER BY id DESC LIMIT 2"
            ).fetchall()
            conn.close()
        except Exception as e:
            self.hist_box.append(f"\n  DB error: {e}")
            return

        if len(rows) < 2:
            self.hist_box.append("\n  Need at least 2 scans to diff. Run another scan.")
            return

        id_new, ts_new, target_new, data_new = rows[0]
        id_old, ts_old, target_old, data_old = rows[1]
        r_new = _json.loads(data_new)
        r_old = _json.loads(data_old)
        ...
        w("═" * 72, ACCENT)
        w("  SIRA v10.0 — SCAN DIFF REPORT", ACCENT)
        w("═" * 72, ACCENT)
        w(f"  SCAN A (older) #{id_old}: {ts_old[:16]}  {target_old}", TEXT_SECONDARY)
        w(f"  SCAN B (newer) #{id_new}: {ts_new[:16]}  {target_new}", TEXT_SECONDARY)
        w("")

        # Port diff
        ports_old = {p["port"] for p in r_old.get("open_ports", [])}
        ports_new = {p["port"] for p in r_new.get("open_ports", [])}
        new_ports   = ports_new - ports_old
        closed_ports = ports_old - ports_new

        w("  ── PORT CHANGES ─────────────────────────────────────────────", ACCENT)
        if new_ports:
            w(f"  🔴 NEW OPEN PORTS (+{len(new_ports)}):", CRITICAL)
            for p_num in sorted(new_ports):
                p_info = next((p for p in r_new.get("open_ports",[]) if p["port"]==p_num), {})
                w(f"      + {p_num}/{p_info.get('proto','tcp')}  {p_info.get('service','?')}  v{p_info.get('version','?')}", HIGH)
        if closed_ports:
            w(f"  🟢 PORTS NOW CLOSED (-{len(closed_ports)}):", LOW)
            for p_num in sorted(closed_ports):
                w(f"      - {p_num}", SUCCESS)
        if not new_ports and not closed_ports:
            w("  ✓ No port changes between scans.", LOW)
        w("")

        # CVE diff
        cves_old = {v["id"] for v in r_old.get("vulns", [])}
        cves_new = {v["id"] for v in r_new.get("vulns", [])}
        new_cves    = cves_new - cves_old
        fixed_cves  = cves_old - cves_new

        w("  ── CVE CHANGES ──────────────────────────────────────────────", ACCENT)
        if new_cves:
            w(f"  🔴 NEW CVEs (+{len(new_cves)}):", CRITICAL)
            for cve_id in sorted(new_cves):
                v_info = next((v for v in r_new.get("vulns",[]) if v["id"]==cve_id), {})
                sev_clr = {"CRITICAL":CRITICAL,"HIGH":HIGH,"MEDIUM":MEDIUM,"LOW":LOW}.get(v_info.get("severity","").upper(),TEXT_SECONDARY)
                w(f"      + {cve_id}  [{v_info.get('severity','?')}]  score:{v_info.get('score','?')}", sev_clr)
        if fixed_cves:
            w(f"  🟢 CVEs RESOLVED (-{len(fixed_cves)}):", LOW)
            for cve_id in sorted(fixed_cves):
                w(f"      - {cve_id}", SUCCESS)
        if not new_cves and not fixed_cves:
            w("  ✓ No CVE changes between scans.", LOW)
        w("")

        # Risk score comparison
        rs_old = self._calculate_risk_score(r_old)
        rs_new = self._calculate_risk_score(r_new)
        delta  = rs_new["score"] - rs_old["score"]
        delta_clr = CRITICAL if delta > 0 else (SUCCESS if delta < 0 else TEXT_SECONDARY)
        delta_sym = f"▲ +{delta}" if delta > 0 else (f"▼ {delta}" if delta < 0 else "= 0 (no change)")
        w("  ── RISK SCORE COMPARISON ────────────────────────────────────", ACCENT)
        w(f"  Scan A: {rs_old['score']}/100  [{rs_old['grade']}]", TEXT_SECONDARY)
        w(f"  Scan B: {rs_new['score']}/100  [{rs_new['grade']}]", TEXT_SECONDARY)
        w(f"  Delta : {delta_sym}", delta_clr)
        w("")
        w("═" * 72, ACCENT)
    ...
    def clear_scan_history(self):
        ...
        if not db_path.exists():
            return
        reply = QMessageBox.question(self, "Clear History", "Delete all scan history records?",
            QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No)
        if reply == QMessageBox.StandardButton.Yes:
            sqlite3.connect(str(db_path)).execute("DELETE FROM scans").connection.commit()
            self.hist_list.clear()
            self.hist_box.clear()
            self.append_log("[✓] Scan history cleared.", "success")

    # ──────────────────────────────────────────
    #  GEOIP INTELLIGENCE (v10.0)
    # ──────────────────────────────────────────
    def run_geoip_lookup(self):
        target = self.geo_input.text().strip()
        if not target:
            target = self.target_input.text().strip()
            if target:
                self.geo_input.setText(target)
        if not target:
            self.geo_box.append("  Enter an IP or hostname to look up.")
            return
        self._do_geoip_lookup(target, self.geo_box)

    def quick_geoip(self):
        """GeoIP lookup for current scan target."""
        target = self.target_input.text().strip()
        if not target:
            self.append_log("[-] Enter a target first.", "error")
            return
        for i in range(self.tabs.count()):
            if "GeoIP" in self.tabs.tabText(i):
                self.tabs.setCurrentIndex(i)
                self.geo_input.setText(target)
                break
        self._do_geoip_lookup(target, self.geo_box)

    def _do_geoip_lookup(self, target: str, box: "QTextEdit"):
        ...
        w("  ⏳ Looking up geolocation...", TEXT_SECONDARY)
        ...
        try:
            ...
            box.clear()
            w(f"  [-] GeoIP lookup failed: {e}", CRITICAL)
            return

        box.clear()
        if data.get("status") != "success":
            w(f"  [-] GeoIP API error: {data.get('message','unknown')}", CRITICAL)
            w(f"  Note: Private/RFC-1918 addresses return no results.", TEXT_SECONDARY)
            return

        w("═" * 60, ACCENT)
        w("  SIRA v10.0 — GEOIP INTELLIGENCE REPORT", ACCENT)
        w("═" * 60, ACCENT)
        w(f"  Query IP       : {data.get('query','—')}", TEXT_PRIMARY)
        w(f"  Country        : {data.get('country','—')}  [{data.get('countryCode','?')}]", ACCENT)
        w(f"  Region         : {data.get('regionName','—')}", TEXT_PRIMARY)
        w(f"  City           : {data.get('city','—')}", TEXT_PRIMARY)
        w(f"  Postal Code    : {data.get('zip','—')}", TEXT_SECONDARY)
        w(f"  Coordinates    : {data.get('lat','?')}, {data.get('lon','?')}", TEXT_SECONDARY)
        w(f"  Timezone       : {data.get('timezone','—')}", TEXT_SECONDARY)
        w("")
        w(f"  ISP            : {data.get('isp','—')}", INFO)
        w(f"  Organization   : {data.get('org','—')}", INFO)
        w(f"  AS Number      : {data.get('as','—')}", TEXT_SECONDARY)
        w("")

        # Threat context
        w("  ── THREAT CONTEXT ──────────────────────────────────────", ACCENT)
        country = data.get("countryCode", "")
        isp = data.get("isp", "").lower()
        org = data.get("org", "").lower()
        if any(x in isp + org for x in ["amazon", "google", "azure", "cloudflare", "digitalocean", "linode", "vultr", "ovh"]):
            w(f"  ⚠ Hosted on cloud/VPS infrastructure — may be a C2 or attack proxy", WARNING)
        if any(x in isp + org for x in ["tor", "vpn", "proxy", "hosting", "datacenter"]):
            w(f"  ⚠ Anonymization/hosting provider detected", WARNING)
        w(f"  Map: https://maps.google.com/?q={data.get('lat')},{data.get('lon')}", TEXT_SECONDARY)
        w("")
        w("═" * 60, ACCENT)

    # ──────────────────────────────────────────
    #  PDF REPORT EXPORT (v10.0)
    # ──────────────────────────────────────────
    def export_pdf_report(self):
        """Export a professional PDF report using reportlab (if available)."""
        if not self.results:
            QMessageBox.warning(self, "No Data", "Run a scan first before exporting a PDF report.")
            return
        ...
        path, _ = QFileDialog.getSaveFileName(
            self, "Export PDF Report", f"SIRA_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.pdf",
            "PDF (*.pdf)"
        )
        if not path:
            return
        ...
        if not REPORTLAB:
            # Fallback: rich HTML-based PDF simulation via basic text
            QMessageBox.information(self, "reportlab not installed",
                "reportlab is not installed. Install it with:\n\n  pip install reportlab\n\n"
                "Falling back to HTML report instead.")
            html_path = path.replace(".pdf", ".html")
            self._export_html(html_path)
            self.append_log(f"[!] PDF skipped (no reportlab) — HTML saved: {html_path}", "warning")
            return
        ...
        try:
            ...
            ports = r.get("open_ports", [])
            vulns = r.get("vulns", [])
            rs    = self._calculate_risk_score(r)
            ...
            title_style   = ParagraphStyle("title",   fontSize=22, fontName="Helvetica-Bold",
                                           textColor=colors.HexColor("#00e5ff"), spaceAfter=6, alignment=TA_CENTER)
            sub_style     = ParagraphStyle("sub",     fontSize=11, fontName="Helvetica",
                                           textColor=colors.HexColor("#64748b"), spaceAfter=4, alignment=TA_CENTER)
            heading_style = ParagraphStyle("heading", fontSize=13, fontName="Helvetica-Bold",
                                           textColor=colors.HexColor("#00e5ff"), spaceBefore=10, spaceAfter=4)
            body_style    = ParagraphStyle("body",    fontSize=9, fontName="Helvetica",
                                           textColor=colors.HexColor("#e2e8f0"), spaceAfter=3)

            grade_clr_map = {
                "CRITICAL": "#ef4444", "HIGH": "#f97316",
                "MEDIUM": "#eab308", "LOW": "#22c55e", "MINIMAL": "#10b981"
            }
            grade_hex = grade_clr_map.get(rs["grade"], "#64748b")

            story = []

            # Title
            story.append(Spacer(1, 6*mm))
            story.append(Paragraph("◈  SIRA v10.0", title_style))
            story.append(Paragraph("Security Intelligence &amp; Recon Agent — Scan Report", sub_style))
            story.append(HRFlowable(width="100%", thickness=1, color=colors.HexColor("#00e5ff"), spaceAfter=8))

            # Meta
            meta = [
                ["Target",   r.get("target","—"),    "IP",       r.get("ip","—")],
                ["Started",  r.get("start_time","—"), "Duration", f"{r.get('scan_duration',0)}s"],
                ["Ports",    str(len(ports)),          "CVEs",     str(len(vulns))],
                ["Risk Score", f"{rs['score']}/100",   "Grade",    rs["grade"]],
            ]
            meta_tbl = Table(meta, colWidths=[35*mm, 55*mm, 35*mm, 55*mm])
            meta_tbl.setStyle(TableStyle([
                ...
                ("FONTSIZE",   (0,0), (-1,-1), 9),
                ("GRID",       (0,0), (-1,-1), 0.5, colors.HexColor("#1e2d42")),
                ("ROWBACKGROUNDS", (0,0), (-1,-1), [colors.HexColor("#111827"), colors.HexColor("#0d1321")]),
                ("PADDING",    (0,0), (-1,-1), 5),
            ]))
            ...
            risk_data = [
                ...
                ["CVE Severity",   str(rs["cve_pts"]),      "50", "50%"],
                ["Port Risk",      str(rs["port_pts"]),     "30", "30%"],
                ["Exposure",       str(rs["exposure_pts"]), "20", "20%"],
                ["TOTAL",          str(rs["score"]),        "100", "—"],
            ]
            risk_tbl = Table(risk_data, colWidths=[70*mm, 30*mm, 30*mm, 30*mm])
            risk_tbl.setStyle(TableStyle([
                ("BACKGROUND",  (0,0), (-1,0), colors.HexColor("#004d66")),
                ("TEXTCOLOR",   (0,0), (-1,0), colors.HexColor("#00e5ff")),
                ("BACKGROUND",  (0,-1),(-1,-1),colors.HexColor("#1e2d42")),
                ("TEXTCOLOR",   (0,-1),(-1,-1),colors.HexColor(grade_hex)),
                ("FONTNAME",    (0,0), (-1,0), "Helvetica-Bold"),
                ("FONTNAME",    (0,-1),(-1,-1),"Helvetica-Bold"),
                ("FONTNAME",    (0,1), (-1,-2),"Helvetica"),
                ("FONTSIZE",    (0,0), (-1,-1), 9),
                ("TEXTCOLOR",   (0,1), (-1,-2), colors.HexColor("#e2e8f0")),
                ("BACKGROUND",  (0,1), (-1,-2), colors.HexColor("#0a0e17")),
                ("GRID",        (0,0), (-1,-1), 0.5, colors.HexColor("#1e2d42")),
                ("PADDING",     (0,0), (-1,-1), 5),
            ]))
            ...
            # Open ports
            story.append(Paragraph(f"Open Ports ({len(ports)})", heading_style))
            if ports:
                port_data = [["Port", "Protocol", "Service", "Version", "Banner"]]
                for p in ports[:50]:
                    port_data.append([
                        str(p["port"]), p.get("proto","tcp").upper(),
                        p["service"], p.get("version","N/A"),
                        p["banner"][:60] or "—"
                    ])
                port_tbl = Table(port_data, colWidths=[18*mm, 20*mm, 28*mm, 30*mm, 84*mm])
                port_tbl.setStyle(TableStyle([
                    ...
                    ("FONTSIZE",   (0,0), (-1,-1), 8),
                    ("TEXTCOLOR",  (0,1), (0,-1), colors.HexColor("#00e5ff")),
                    ("TEXTCOLOR",  (1,1), (-1,-1),colors.HexColor("#e2e8f0")),
                    ("ROWBACKGROUNDS",(0,1),(-1,-1),[colors.HexColor("#0a0e17"),colors.HexColor("#0d1321")]),
                    ("GRID",       (0,0), (-1,-1), 0.5, colors.HexColor("#1e2d42")),
                    ("PADDING",    (0,0), (-1,-1), 4),
                ]))
            ...
            # Vulnerabilities
            story.append(Paragraph(f"Vulnerabilities / CVEs ({len(vulns)})", heading_style))
            if vulns:
                vuln_data = [["CVE ID", "Service", "Port", "Score", "Severity", "Exploit"]]
                for v in vulns[:50]:
                    vuln_data.append([
                        v["id"], v["service"], str(v["port"]),
                        str(v["score"]), v["severity"],
                        "YES" if v.get("exploit_available") else "No"
                    ])
                vuln_tbl = Table(vuln_data, colWidths=[42*mm, 28*mm, 18*mm, 18*mm, 26*mm, 18*mm])
                vuln_tbl.setStyle(TableStyle([
                    ("BACKGROUND", (0,0), (-1,0), colors.HexColor("#3d0f0f")),
            ...
            story.append(HRFlowable(width="100%", thickness=1, color=colors.HexColor("#1e2d42")))
            story.append(Paragraph(
                f"Generated by SIRA v10.0 — Security Intelligence & Recon Agent  ·  {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}",
                ParagraphStyle("footer", fontSize=8, textColor=colors.HexColor("#374151"), alignment=TA_CENTER)
            ))

            doc.build(story)
            self.append_log(f"[✓] PDF report saved: {path}", "success")
            QMessageBox.information(self, "PDF Exported", f"Report saved to:\n{path}")

        except Exception as e:
            self.append_log(f"[-] PDF export error: {e}", "error")
            QMessageBox.warning(self, "PDF Error", f"PDF export failed:\n{e}\n\nTry: pip install reportlab")

    # ──────────────────────────────────────────
    #  AI THREAT BRIEF (v10.0)
    # ──────────────────────────────────────────
    def generate_ai_threat_brief(self):
        """Generate an AI threat brief using Anthropic Claude API."""
        import os
        if not self.results:
            QMessageBox.warning(self, "No Data", "Run a scan first before generating an AI brief.")
            return

        api_key = os.environ.get("ANTHROPIC_API_KEY", "")
        if not api_key:
            QMessageBox.warning(self, "API Key Missing",
                "Set the ANTHROPIC_API_KEY environment variable to use this feature.\n\n"
                "export ANTHROPIC_API_KEY=sk-ant-...")
            return

        self.ai_btn.setEnabled(False)
        self.ai_btn.setText("⏳  Generating brief...")
        self.ai_box.clear()
        self.ai_box.setTextColor(QColor(TEXT_SECONDARY))
        self.ai_box.append("  Connecting to Anthropic Claude API...")
        QApplication.processEvents()
        ...
        port_summary = ", ".join(f"{p['port']}/{p.get('proto','tcp')} ({p['service']} v{p.get('version','?')})" for p in ports[:15])
        cve_summary  = ", ".join(f"{v['id']} [{v['severity']} {v['score']}]" for v in vulns[:10])

        prompt = f"""You are a senior cybersecurity analyst. A network scan has been completed.
Provide a concise, professional threat brief. Be specific, actionable, and direct.

TARGET: {r.get('target','N/A')} ({r.get('ip','N/A')})
SCAN DATE: {r.get('start_time','N/A')}
RISK SCORE: {rs['score']}/100 ({rs['grade']})

OPEN PORTS & SERVICES:
{port_summary or 'None found'}

CVEs DETECTED:
{cve_summary or 'None found'}

EXPLOITABLE CVEs: {rs['exploitable']}
CRITICAL CVEs: {rs['critical_cves']}

Write a threat brief with:
1. Executive Summary (2-3 sentences, non-technical)
2. Key Findings (bullet points with severity)
3. Immediate Actions Required (prioritized by urgency)
4. Longer-Term Recommendations
5. Overall Threat Assessment

Keep it under 500 words. Use plain text, no markdown."""
        ...
        def _check():
            ...
            self.ai_btn.setText("🤖  GENERATE AI THREAT BRIEF")
            self.ai_box.clear()
            if result_holder.get("err"):
                self.ai_box.setTextColor(QColor(CRITICAL))
                self.ai_box.append(f"  API Error: {result_holder['err']}")
            else:
                text = result_holder.get("text","No response")
                lines = text.split("\n")
                for line in lines:
                    line = line.strip()
                    if not line:
                    ...
                    if line.startswith(("1.", "2.", "3.", "4.", "5.")):
                        self.ai_box.setTextColor(QColor(ACCENT))
                        self.ai_box.append(f"  {line}")
                    elif "URGENT" in line.upper() or "CRITICAL" in line.upper():
                        self.ai_box.setTextColor(QColor(CRITICAL))
                        self.ai_box.append(f"  {line}")
                    elif "IMMEDIATE" in line.upper() or "HIGH" in line.upper():
                        self.ai_box.setTextColor(QColor(HIGH))
                        self.ai_box.append(f"  {line}")
                    else:
                        self.ai_box.setTextColor(QColor(TEXT_PRIMARY))
                        self.ai_box.append(f"  {line}")
...
if __name__ == "__main__":
    ...
    app.setStyle("Fusion")
    app.setApplicationName("SIRA")
    app.setApplicationVersion("9.0")
    app.setOrganizationName("SIRA")
